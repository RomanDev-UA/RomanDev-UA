...
import re
import math
from fpdf import FPDF  # Убедись, что в requirements.txt написано fpdf2
import io

# --- 1. СТИЛИ ---
st.set_page_config(page_title="IRON WORKS v11.0", layout="wide", page_icon="🏗️")
st.markdown("""
    <style>
...
def generate_pdf(report_data):
    ...
    pdf.cell(200, 10, txt=f"ОТЧЕТ: {report_data['name']}", ln=True, align='C')
    pdf.ln(10)
    
    pdf.set_font('CustomArial' if os.path.exists(font_path) else "Helvetica", '', 11)
    for key, value in report_data['metrics'].items():
        pdf.cell(200, 8, txt=f"{key}: {value}", ln=True)
    
    pdf.ln(10)
    pdf.multi_cell(0, 8, txt=f"ИНЖЕНЕРНОЕ ЗАКЛЮЧЕНИЕ:\n{report_data['advice']}")
...
if calc and all_prices:
    ...
    if is_sheet:
        ...
        report_results["metrics"] = {"Тип": "Лист", "Кол-во": f"{Qty} шт", "Вес": f"{total_w:.1f} кг", "Малярка": f"{paint:.2f} м2"}
        ...
        paint = (0.2 * m_real) # Пример
        
        report_results["metrics"] = {"Тип": "Каркас", "Длина": f"{L_f} мм", "Вес": f"{w_total:.1f} кг", "Малярка": f"{paint:.2f} м2"}
    ...
    # ИНЖЕНЕРКА
    is_crit = thick < 2.0
    advice = f"Толщина {thick}мм. {'Варить аккуратно, металл тонкий!' if is_crit else 'Металл стабильный.'} Проверьте диагонали."
    report_results["advice"] = advice
    st.markdown(f'<div class="eng-card"><b>Инженер:</b> {advice}</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.download_button("📥 СКАЧАТЬ PDF", data=pdf_bytes, file_name="iron_report.pdf", mime="application/pdf")
