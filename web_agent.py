...
# --- 1. НАСТРОЙКИ И СТИЛИ ---
st.set_page_config(page_title="IRON WORKS v14.2", layout="wide", page_icon="🏗️")
st.markdown("""
    <style>
...
st.markdown('<div class="main-header"><h1 style="color: white; margin: 0; font-weight: 900;">🏗️ IRON WORKS | Full 3D Engineering v14.2</h1></div>', unsafe_allow_html=True)
...
if db:
    ...
    if calc_btn:
        ...
        st.markdown(f'<div class="eng-card"><b>МАТЕРИАЛ:</b> {sel}<br><b>ТОЛЩИНА:</b> {item["thick"]}мм<br><b>СВАРКА:</b> Ток {85 + item["thick"]*5:.0f}А. Режим сварки: полуавтомат/электрод.</div>', unsafe_allow_html=True)

        # --- ПОЛНОЦЕННЫЙ 3D КАРКАС ---
        st.subheader("📦 3D Визуализация")
        # Координаты всех 12 ребер (путь без отрыва руки для отрисовки каркаса)
        x_coords = [0, lx, lx, 0, 0, 0, lx, lx, 0, 0, None, lx, lx, None, lx, lx, None, 0, 0]
        y_coords = [0, 0, ly, ly, 0, 0, 0, ly, ly, 0, None, 0, 0, None, ly, ly, None, ly, ly]
        z_coords = [0, 0, 0, 0, 0, lz, lz, lz, lz, lz, None, 0, lz, None, 0, lz, None, 0, lz]

        fig = go.Figure(data=[go.Scatter3d(x=x_coords, y=y_coords, z=z_coords, mode='lines', line=dict(color='#00c6ff', width=7))])
        fig.update_layout(scene=dict(xaxis_title='L', yaxis_title='W', zaxis_title='H', aspectmode='data'), height=550, margin=dict(l=0,r=0,b=0,t=0), paper_bgcolor='#0e1117')
        ...
        # Код для AutoCAD
        st.subheader("📝 Скрипт для AutoCAD")
