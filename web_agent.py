...
# --- 1. НАСТРОЙКИ СТИЛЕЙ ---
st.set_page_config(page_title="IRON WORKS v13.6", layout="wide", page_icon="🏗️")
st.markdown("""
    <style>
...
st.markdown('<div class="main-header"><h1 style="color: white; margin: 0; font-weight: 900;">🏗️ IRON WORKS | Full Engineering v13.6</h1><p style="color: #00c6ff; font-size: 18px; margin: 0;">Профессиональный расчет и генерация PDF</p></div>', unsafe_allow_html=True)

# --- 2. ГЕНЕРАТОР PDF (С защитой от ошибок кодировки) ---
def create_pdf(res, weld_info):
    pdf = FPDF()
    pdf.add_page()
    
    # Автоматическая подгрузка шрифта
    font_path = "DejaVuSans.ttf"
    if not os.path.exists(font_path):
        font_url = "https://github.com/matomo-org/travis-scripts/raw/master/fonts/DejaVuSans.ttf"
        urllib.request.urlretrieve(font_url, font_path)
    
    # Подключаем юникод-шрифт
    ...
    pdf.multi_cell(0, 7, txt=f"Сложность: {weld_info['crit']}\nТок: {weld_info['amp']}\nМетод: {weld_info['meth']}")
    
    # Сохраняем во временный файл, чтобы избежать крашей encode() в Streamlit
    temp_file = "temp_smeta.pdf"
    pdf.output(temp_file)
    
    with open(temp_file, "rb") as f:
        pdf_data = f.read()
        
    os.remove(temp_file) # Подчищаем за собой
    return pdf_data
...
def get_welding_advice(thick):
    if thick < 1.6:
        return {"crit": "ВЫСОКАЯ (Риск прожога)", "amp": "30-55A", "el": "1.6-2.0мм", "meth": "Точечно, с паузами."}
    elif 1.6 <= thick < 3.0:
        return {"crit": "СРЕДНЯЯ", "amp": "65-95A", "el": "2.5-3.0мм", "meth": "Прерывистый шов."}
    else:
...
if db:
    ...
    if calc_btn:
        ...
        st.download_button(label="📥 СКАЧАТЬ СМЕТУ (PDF)", data=pdf_bytes, file_name=f"Smeta_Order.pdf", mime="application/pdf", use_container_width=True)

st.code(f"(defun c:IronCAD () (command \"_BOX\" '(0 0 0) \"_L\" {L if 'is_sheet' in locals() and is_sheet else 2000} {W if 'is_sheet' in locals() and is_sheet else 1000} {item['thick'] if 'is_sheet' in locals() and is_sheet else 1200}) (princ))")
