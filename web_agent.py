...
# --- 1. СТИЛИ ---
st.set_page_config(page_title="IRON WORKS v13.9", layout="wide", page_icon="🏗️")
st.markdown("""
    <style>
...
st.markdown('<div class="main-header"><h1 style="color: white; margin: 0; font-weight: 900;">🏗️ IRON WORKS | Final v13.9</h1></div>', unsafe_allow_html=True)

# --- 2. ГЕНЕРАТОР PDF (ИСПРАВЛЕННЫЙ ВЫВОД БАЙТОВ) ---
def create_pdf(res, weld_info):
    ...
    pdf.cell(200, 10, txt=f"Material: {res['name']}", ln=True)
    ...
    pdf.ln(10)
    pdf.multi_cell(0, 10, txt=f"Welding: {weld_info['meth']}")
    
    # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: возвращаем байты, а не объект
    return pdf.output()
...
if db:
    with st.sidebar:
        ...
        mode = st.radio("Тип цены:", ["Розница", "Опт"])
        ...
        if is_sheet:
            ...
            rl, rw, rh = st.number_input("L (мм)", 2000.0), st.number_input("W (мм)", 1000.0), st.number_input("H (мм)", 1200.0)
    ...
    if calc_btn:
        ...
        st.markdown(f'<div class="eng-card"><b>МЕТАЛЛ:</b> {sel}<br><b>ТОЛЩИНА:</b> {item["thick"]}мм<br><b>СВАРКА:</b> Электрод 3мм, ток 90-110А.</div>', unsafe_allow_html=True)

        # 3D
        fig = go.Figure(data=[go.Scatter3d(x=[0, rl if not is_sheet else L], y=[0, rw if not is_sheet else W], z=[0, rh if not is_sheet else 2], mode='lines', line=dict(color='#00c6ff', width=6))])
        ...
        w_advice = {"meth": "Standard welding procedure for structural steel."}
        
        # Получаем байты напрямую из функции
        pdf_data = create_pdf(res_pdf, w_advice)
